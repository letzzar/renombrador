...
| `FILE_MODE`        | *(from destination)*   | Octal override for every moved file. Unset, the file takes the destination folder's mode without the execute/special bits (Linux only). |
| `DIR_MODE`         | *(from destination)*   | Octal override for every destination folder *created* by the service; existing ones are left alone. Unset, a new folder copies the mode of the level above it (Linux only). |
| `PUID` / `PGID`    | *(from destination)*   | Numeric owner/group override. Unset, the result takes the destination folder's `uid:gid`. Requires the container to run as root (Linux only). |

> **Which folder is the reference.** The first level above the destination that already exists — and that has at least one read bit. A folder with none (`000`, `--x--x--x`) is skipped and the search goes one level up. On an ACL-backed share (the norm on a Synology NAS; the `+` in `ls -l` gives it away) the real permissions live in the ACL and the POSIX bits sit at zero, so copying that zero onto a file would make it genuinely unreadable.
...
| Player says `Permission denied` on the moved files | Files used to land with the permissions they had in the downloads folder (often `000`). Since v1.2.2 every moved file and every folder created for it takes the mode and `uid:gid` of the destination folder it lands in, so the result matches the library around it; check the `Permisos:` line in the startup log. Files moved by an older version need a one-off `chmod a+r` on the host. If the destination folder itself is too closed, fix *it* — that is now the reference. Since v1.2.3 a reference folder with no read bits at all is skipped in favour of the level above, so an old `_revisar` sitting at `000` can no longer poison what lands in it. |
...
| `FILE_MODE`        | *(del destino)*        | Fuerza los permisos (octal) de cada archivo movido. Sin definir, el archivo toma el modo de su carpeta de destino sin los bits de ejecución ni los especiales (solo Linux). |
| `DIR_MODE`         | *(del destino)*        | Fuerza los permisos (octal) de las carpetas que *crea* el servicio; las que ya existían no se tocan. Sin definir, una carpeta nueva copia el modo del nivel del que cuelga (solo Linux). |
| `PUID` / `PGID`    | *(del destino)*        | Fuerza el propietario/grupo numérico. Sin definir, el resultado toma el `uid:gid` de la carpeta de destino. Exige que el contenedor corra como root (solo Linux). |

> **Qué carpeta es la referencia.** El primer nivel por encima del destino que ya exista *y* que tenga algún bit de lectura. Una carpeta que no tenga ninguno (`000`, `--x--x--x`) se descarta y se sube un nivel más. En un recurso con ACL —lo normal en un NAS Synology; se reconoce por el `+` de `ls -l`— los permisos de verdad viven en la ACL y los bits POSIX se quedan a cero, así que copiar ese cero a un archivo lo dejaría ilegible de verdad.
...
| El reproductor da `Permission denied` con los archivos movidos | Los archivos llegaban con los permisos que traían de la carpeta de descargas (a menudo `000`). Desde la v1.2.2 cada archivo movido y cada carpeta que se crea para él toman el modo y el `uid:gid` de la carpeta de destino en la que aterrizan, así que el resultado queda igual que la biblioteca que lo rodea; comprueba la línea `Permisos:` del log de arranque. Lo movido por versiones anteriores necesita un `chmod a+r` puntual en el host. Si la carpeta de destino es la que está cerrada, arréglala a ella: ahora es la referencia. Desde la v1.2.3, una carpeta de referencia sin ningún bit de lectura se descarta en favor del nivel de encima, así que una `_revisar` antigua en `000` ya no puede envenenar lo que cae dentro. |
